...
import { uploadImage, runImageEdit, connectProgress, type ProgressEvent, imageUrl } from '@/api/client'
...
export default function ImageEdit() {
  ...
  const submit = async () => {
    ...
    try {
      const { prompt_id, client_id } = await runImageEdit({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Prompt */}
      <div className="space-y-1.5">
        <label className="text-xs text-muted-foreground uppercase tracking-widest">Edit Instruction</label>
        <textarea
          value={prompt}
